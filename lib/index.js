...
function makeEnumerable(obj, props) {
  const proto = obj.prototype;
  for (let pn of props) {
    const pd = Object.getOwnPropertyDescriptor(proto, pn);
    pd.enumerable = true;
    Object.defineProperty(proto, pn, pd);
  }
}
...
function toSkMatrix() {
  try {
    ...
    throw new TypeError(`Invalid transform matrix argument(s): ${[...arguments]}`);
  }
}
...
class App extends RustClass{
  ...
  static {
    makeEnumerable(App, ["fps", "running", "windows"])
  }
...
class Window extends EventEmitter{
  ...
  static {
    makeEnumerable(Window, this.#kwargs)
  }
...
class Canvas extends RustClass{
  ...
  static {
    makeEnumerable(Canvas, ["width", "height", "size", "async", "gpu", "pages"])
  }
...
class CanvasPattern extends RustClass{
  constructor(canvas, src, repeat){
    ...
    if (src instanceof Image){
      this.init('from_image', core(src), core(canvas), repeat)
...
class CanvasRenderingContext2D extends RustClass{
  ...
  static ENUM_PROPS = [
    "canvas", "currentTransform", "direction", "fillStyle", "filter", "font", "fontVariant",
    "globalAlpha", "globalCompositeOperation", "imageSmoothingEnabled", "imageSmoothingQuality",
    "lineCap", "lineDashFit", "lineDashMarker", "lineDashOffset", "lineJoin", "lineWidth", "miterLimit",
    "shadowBlur", "shadowColor", "shadowOffsetX", "shadowOffsetY", "strokeStyle",
    "textAlign", "textBaseline", "textTracking", "textWrap",
  ]

  static {
    makeEnumerable(CanvasRenderingContext2D, this.ENUM_PROPS)
  }
  ...
  // -- shaders ---------------------------------------------------------------
  createPattern(image, repetition){ return new CanvasPattern(this, image, repetition) }
  ...
  drawImage(image, ...coords){
    if (image instanceof Image){
      ...
      // Keep for compatibility; see https://github.com/samizdatco/skia-canvas/issues/77
  ...
  drawCanvas(canvas, ...coords){
    if (canvas instanceof Canvas){
      this.ƒ('drawCanvas', core(canvas.getContext('2d')), ...coords)
    }else{
      this.drawImage(canvas, ...coords)
    }
  }
  ...
  [REPR](depth, options) {
    let info = {}
    if (depth > 0 ){
      for (var prop of CanvasRenderingContext2D.ENUM_PROPS){
...
class FontLibrary extends RustClass {
  static {
    makeEnumerable(FontLibrary, ["families"])
  }
...
class Image extends RustClass {
  ...
  static {
    makeEnumerable(Image, ["width", "height", "naturalWidth", "naturalHeight", "complete", "src"])
  }
  ...
  constructor(...args) {
    ...
    if (typeof args[0] == 'number' || args.length == 2) {
      ...
      if (this.#options.width != undefined)
        this.width = parseFloat(this.#options.width)
      if (this.#options.height != undefined)
        this.height = parseFloat(this.#options.height)
    }
  }
  ...
  set src(src){
    const noop = () => {},
      ...
      passthrough = fn => arg => { (fn||noop)(arg); this.#fetch = null; }
    const setData = d => {
      let ok;
      if (this.#options.type === 'svg')
        ok = this.ƒ("load_svg", d)
      else if (this.#options.raw)
        ok = this.ƒ("load_pixel_data", d, this.#options.raw)
      else
        ok = this.prop("data", d)
      ...
      if (ok)
        ...
        onerror(new Error("Could not decode image data"))
    }
    ...
    if (Buffer.isBuffer(src)) {
      ...
      if (!mime || !enc) {
        onerror(new Error(`Invalid data URI header`))
        return
      }
      ...
      get.concat(src, (err, res, data) => {
        ...
        if (err) {
          ...
          onerror(new Error(`Returned image data was missing or empty.`))
        } else {
          if (src.toLowerCase().endsWith(".svg") || (res && res.headers && (res.headers['content-type'] || "").includes("svg")))
            this.#options.type = 'svg'
          setData(data)
        }
      })
      ...
      if (data && src.toLowerCase().endsWith(".svg"))
        this.#options.type = 'svg'
    }
...
class Path2D extends RustClass{
  ...
  static {
    makeEnumerable(Path2D, ["bounds", "d", "edges"])
  }
...
module.exports = {
  ...
  TextMetrics, Image, ImageData, Path2D, loadImage, colorTypeBytesPerPixel, ...geometry,
