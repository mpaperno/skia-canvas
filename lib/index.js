...
class Image extends RustClass {
  ...
  set src(src){
    var onload = img => fetch.emit('ok', img),
        onerror = err => fetch.emit('err', err),
        passthrough = fn => arg => { fn && fn(arg); delete this._fetch },
        data

    if (this._fetch) this._fetch.removeAllListeners()
    let fetch = this._fetch = new EventEmitter()
        .once('ok', passthrough(this.onload?.bind(this)))
        .once('err', passthrough(this.onerror?.bind(this)))
